...
public class UIPatientMenu {
    ...
    private static void showBookAppointmentMenu() {
        ...
        do {
            ...
            Map<Integer, Doctor> doctorAvalaibleSelected = doctors.get(responseDateSelected);
            Integer indexDate = 0;
            Doctor doctorSelected = new Doctor("","");

            for (Map.Entry<Integer,Doctor> doc :doctorAvalaibleSelected.entrySet()) {
            ...
            System.out.println("Confirm your appointmens: \n1. Yes \n2. Change Data");
            response = Integer.valueOf(sc.nextLine());

            if (response == 1) {
                UIMenu.patinetLogged.addAppointmentDoctors(doctorSelected, doctorSelected.getAvailableAppointments().get(indexDate).getDate(null),
    ...
    private static void showPatientMyAppontments() {
        int response = 0;
        do {
            ...
            for (int i = 0; i < UIMenu.patinetLogged.getAppointmentDoctors().size(); i++) {
                int j = i + 1;
                System.out.println(j + ". " + "Date " + UIMenu.patinetLogged.getAppointmentDoctors().get(i).getDate() +
                        "Time " + UIMenu.patinetLogged.getAppointmentDoctors().get(i).getTime() +
                        "\n Doctors: " + UIMenu.patinetLogged.getAppointmentDoctors().get(i).getDoctor());
            }
